...
impl CryptoError {
    ...
    pub fn crypto_error(msg: &str) -> Self {
        CryptoError::CryptoError(msg.to_string())
    }

    pub fn serialization_error(msg: &str) -> Self {
        CryptoError::SerializationError(msg.to_string())
    }
}
...
pub trait CryptoService: Send + Sync {
    ...
    /// 鍵ペアの生成
    fn generate_keypair(&self) -> CryptoResult<(SecretKey, PublicKey)>;
}
...
impl CryptoServiceImpl {
    ...
    fn deserialize_secret_key(&self, key: &SecretKey) -> CryptoResult<umbral_pre::SecretKey> {
        ...
        // SecretKeyに変換
        let result = umbral_pre::SecretKey::try_from_be_bytes(&secret_bytes).map_err(|_| {
            CryptoError::crypto_error("Failed to deserialize secret key")
        });
    ...
    fn deserialize_public_key(&self, key: &PublicKey) -> CryptoResult<umbral_pre::PublicKey> {
        bincode::deserialize(&key.key_data).map_err(|_| {
            CryptoError::crypto_error("Failed to deserialize key")
        })
    }

    /// Capsuleからumbral_pre::Capsuleを復元
    fn deserialize_capsule(&self, capsule: &Capsule) -> CryptoResult<umbral_pre::Capsule> {
        bincode::deserialize(&capsule.data).map_err(|_| {
            CryptoError::crypto_error("Failed to deserialize capsule")
        })
    }
}
...
impl CryptoService for CryptoServiceImpl {
    fn split_secret_shamir(
        ...
        if threshold < constants::MIN_THRESHOLD {
            return Err(CryptoError::validation_error(
                "Threshold below minimum requirement",
            ));
        }

        if total_shares > constants::MAX_SHARES {
            return Err(CryptoError::validation_error("Too many shares requested"));
        }
        ...
        if secret.is_empty() {
            return Err(CryptoError::validation_error("Secret cannot be empty"));
        }
        ...
        let shares: Vec<Vec<u8>> = create_shares(&padded_secret, total_shares, threshold)
            .map_err(|_| CryptoError::crypto_error("Failed to create shares"))?;
    ...
    fn reconstruct_secret_shamir(
        ...
        // ライブラリを使用してシェアを結合
        let recovered = combine_shares(&share_vecs).map_err(|_| {
            CryptoError::crypto_error("Failed to combine shares")
        })?;
        ...
        match recovered {
            Some(data) => {
                // パディングを除去して元のデータを取得
                if data.is_empty() {
                    return Err(CryptoError::crypto_error("Recovered data is empty"));
                }

                let original_len = data[0] as usize;
                if original_len == 0 || original_len > DATA_SIZE - 1 {
                    return Err(CryptoError::crypto_error(
                        "Invalid recovered data format"
                    ));
                }
            ...
            None => Err(CryptoError::crypto_error(
    ...
    fn create_pre_capsule(
        ...
        if public_key.key_data.is_empty() {
            return Err(CryptoError::validation_error("Invalid public key"));
        }

        if plaintext.is_empty() {
            return Err(CryptoError::validation_error("Plaintext cannot be empty"));
        }
        ...
        let (umbral_capsule, ciphertext_box) = umbral_pre::encrypt(&umbral_pk, plaintext)
            .map_err(|_| CryptoError::crypto_error("Encryption failed"))?;

        // Capsuleをシリアライズして保存
        let capsule_bytes: Vec<u8> = bincode::serialize(&umbral_capsule).map_err(|_| {
            CryptoError::crypto_error("Failed to serialize capsule")
        })?;
    ...
    fn generate_reencryption_key(
        ...
        if owner_secret_key.key_data.is_empty() {
            return Err(CryptoError::validation_error("Invalid owner secret key"));
        }
    ...
    fn create_kfrags(
        ...
        {
            return Err(CryptoError::validation_error("Invalid reencryption key"));
        }
        ...
        let verification_data = VerificationData {
            verifying_pk: bincode::serialize(&self.verifying_key).map_err(|_| {
                CryptoError::crypto_error("Failed to serialize key")
            })?,
            delegating_pk: bincode::serialize(&umbral_delegating_pk).map_err(|_| {
                CryptoError::crypto_error("Failed to serialize key")
            })?,
            receiving_pk: reencryption_key.receiving_pk_data.to_vec(),
        };
        let verification_bytes = bincode::serialize(&verification_data).map_err(|_| {
            CryptoError::crypto_error("Failed to serialize verification data")
        })?;
        ...
        for (index, verified_kfrag) in verified_kfrags.iter().enumerate() {
            ...
            let kfrag_bytes = kfrag.to_bytes().map_err(|_| {
                CryptoError::crypto_error("Failed to serialize fragment")
            })?;
    ...
    fn proxy_reencrypt(
        ...
        if kfrag.key_data.is_empty() {
            return Err(CryptoError::validation_error("Invalid key fragment"));
        }

        if capsule.data.is_empty() {
            return Err(CryptoError::validation_error("Invalid capsule"));
        }
        ...
        let verification_data = bincode::deserialize::<VerificationData>(&kfrag.verification_data)
            .map_err(|_| {
                CryptoError::crypto_error("Failed to deserialize verification data")
            })?;

        // KeyFragをデシリアライズ
        let umbral_kfrag = umbral_pre::KeyFrag::from_bytes(&kfrag.key_data).map_err(|_| {
            CryptoError::crypto_error("Failed to deserialize fragment")
        })?;

        // 常に署名検証を実行
        let verifying_pk: umbral_pre::PublicKey =
            bincode::deserialize(&verification_data.verifying_pk).map_err(|_| {
                CryptoError::crypto_error("Failed to deserialize key")
            })?;
        let delegating_pk: umbral_pre::PublicKey =
            bincode::deserialize(&verification_data.delegating_pk).map_err(|_| {
                CryptoError::crypto_error("Failed to deserialize key")
            })?;
        ...
        let verified_kfrag = umbral_kfrag
            ...
            .map_err(|_| CryptoError::crypto_error("Failed to verify fragment"))?;
        ...
        let cfrag_bytes = cfrag.to_bytes().map_err(|_| {
            CryptoError::crypto_error("Failed to serialize capsule fragment")
        })?.to_vec();

        // CapsuleFrag検証用データを準備（既存の検証データを再利用）
        let cfrag_verification_bytes = bincode::serialize(&verification_data).map_err(|_| {
            CryptoError::crypto_error("Failed to serialize verification data")
        })?;
    ...
    fn combine_and_decrypt(
        ...
        if original_capsule.data.is_empty() {
            return Err(CryptoError::validation_error("Invalid original capsule"));
        }

        if ciphertext.is_empty() {
            return Err(CryptoError::validation_error("Ciphertext cannot be empty"));
        }
        ...
        for cfrag in cfrags {
            ...
            let verified_cfrag = capsule_frag
                ...
                .map_err(|_| {
                    CryptoError::crypto_error("Failed to verify fragment")
                })?;
        ...
        .map_err(|_| CryptoError::crypto_error("Failed to decrypt"))?;
    ...
    fn generate_keypair(&self) -> CryptoResult<(SecretKey, PublicKey)> {
        ...
        // 公開鍵をシリアライズ
        let pk_bytes: Vec<u8> =
            bincode::serialize(&umbral_pk).map_err(|_| {
                CryptoError::crypto_error("Failed to serialize key")
            })?;
...
mod tests {
    ...
    fn test_proxy_reencrypt_full_flow() {
        ...
        println!("   ✓ 復号成功");
        println!("   復号データ: \"{}\"", std::str::from_utf8(&decrypted).unwrap());
    ...
    fn test_combine_and_decrypt() {
        ...
        if result.is_err() {
            ...
            if let Ok(decrypted) = result {
                assert_ne!(plaintext, &decrypted[..], "閾値未満では正しく復号できないはず");
