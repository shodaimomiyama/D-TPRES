...
Requester Roleは、FORMIXシステムにおいて秘密へのアクセスを要求し、暗号フラグメントを収集して秘密を復元する重要な役割を担います。Requesterは、EVMスマートコントラクトによるアクセス権限の検証と、k-of-n閾値暗号の復号プロセスを調整します。
...
Requester Roleハンドラーは、FORMIXシステムにおける秘密へのアクセスと復元プロセスの中核として、以下の特性を持ちます：
