...
Holder Roleは、D-TPRESシステムにおける分散ストレージと再暗号化の実行を担当する重要なロールです。k-of-n閾値暗号化スキームにおいて、各Holderは再暗号化キーフラグメント（kFrag）を保持し、認証されたアクセス要求に対してプロキシ再暗号化を実行します。
...
Holder Roleハンドラーは、D-TPRESシステムの分散性と信頼性を支える重要な要素として、以下の特性を実現します：
