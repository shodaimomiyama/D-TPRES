# FORMIX アクセス要求ライフサイクル詳細

## 1. はじめに

本ドキュメントは、FORMIX（分散型暗号学的秘密管理ライブラリ）におけるアクセス要求のライフサイクルを詳細に定義します。アクセス要求の作成から完了までの全フェーズ、外部アクセス制御との連携、cFrag収集、エラー処理について説明します。

### 1.1 アクセス要求の重要性

アクセス要求は、FORMIXライブラリの中核的なワークフローです：

- **認可制御**: 外部アクセス制御システムとの連携（検証済み前提）
- **プライバシー保護**: 必要最小限の情報開示
- **監査可能性**: FORMIX内部処理の追跡と検証
...
FORMIXライブラリでは、アクセス制御（pk_A検証）は外部システムで完了済みとして処理します：

- **外部システムの責任**: Requesterの公開鍵pk_Aの正当性検証
- **FORMIX内処理**: 検証済みpk_Aを使用した暗号化処理のみ
- **責任分界**: アクセス制御ロジックは外部、暗号学的処理はFORMIX内部
...
FORMIXのアクセス要求ライフサイクル管理は、PRD.mdの更新に合わせて分散型暗号学的秘密管理ライブラリとして最適化されました。
...
4. **責任分界**: FORMIX内部は純粋な暗号処理、アクセス制御は外部
